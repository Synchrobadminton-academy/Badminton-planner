...
| `FUN_FACTS_CHAT_ID` | Optional: chat for the weekly fun-facts post (defaults to `ALLOWED_CHAT_ID`) |
| `FUN_FACTS_TOPIC_ID` | Optional: topic (thread) ID the weekly fun-facts post goes to |
| `FUN_FACTS_DAY` | Optional: weekday of the weekly fun-facts post, `mon`/`tue`/… (default `mon`) |
| `FUN_FACTS_TIME` | Optional: time of the weekly fun-facts post, 24h Singapore time (default `09:00`) |

## Weekly meeting fun facts

Every week (Monday 9am SGT by default) the receipt bot posts a short
"fun facts" message to the group for the weekly meeting: last week's session
count, court-hours, busiest venue, hardest-working coach, the week ahead, and
one badminton trivia fact (generated by Claude, with a built-in fallback list;
recent facts are remembered in `/fun_facts_history` so they don't repeat).
Anyone in the group can also pull it up on demand with `/funfacts`.
